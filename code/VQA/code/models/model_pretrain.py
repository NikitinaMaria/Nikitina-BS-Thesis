...
class ContrastiveLossBase(nn.Module):
    def __init__(self, temperature=1., cuda=True):
        super().__init__()
    ...
    def forward(self, out_1, out_2):
        ...
        out_2 = torch.transpose(out_2, 0, 1)[:batch_size * 2, :]

        out = torch.cat([out_1, out_2], dim=0)
        # Скалярное произведение
        full = torch.exp(torch.mm(out, out.t().contiguous()) / self.temperature)
        # full = out_1 @ out_2 / self.temperature
...
class DebiasedPosLoss(nn.Module):
    def __init__(self, temperature, cuda=True, tau_plus=0.1):
        super().__init__()
        self.temperature: float = temperature
        self.cuda: bool = cuda
    ...
    def forward(self, out_1, out_2):
        batch_size = out_1.shape[0]
        out_2 = torch.transpose(out_2, 0, 1)[:batch_size, :]
        N = batch_size * 2 - 2
...
class ALBEF(nn.Module):
    ...
    def forward(self, image, image_aug, text, alpha=0):
        ...
        loss_i2t = DebiasedPosLoss(temperature=self.temp)(image_feat, text_feat_all)
        loss_t2i = DebiasedPosLoss(temperature=self.temp)(text_feat, image_feat_all)

        loss_i2i = DebiasedPosLoss(temperature=self.temp)(image_feat, image_feat_all)
        loss_t2t = DebiasedPosLoss(temperature=self.temp)(text_feat, text_feat_all)
